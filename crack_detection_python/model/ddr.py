...
class DAPPM(nn.Module):#planes64 * 16, spp_planes128, planes * 4 
...
class DualResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=19, planes=64, spp_planes=128, head_planes=128, augment=False):
        ...
        self.relu = nn.ReLU(inplace=False)  #[3, 4, 6, 3]
    ...
    def _make_layer(self, block, inplanes, planes, blocks, stride=1):#block, planes, planes, 3
        downsample = None
    ...
    def forward(self, x):
        ...
        x = self.layer1(x)
        layers.append(x) #[layer0]

        x = self.layer2(self.relu(x))
        layers.append(x) #[layer1]

        x = self.layer3_1(self.relu(x))
        layers.append(x) #[layer2]
        x_ = self.layer3_1_(self.relu(layers[1]))  # x_:高分辨率   x：低分辨率
        ...
        a = F.interpolate(x_, size=(h, w), mode="bilinear",align_corners=True) # (shape: (batch_size, num_classes, h, w))
...
def ddrnet():
    model = DualResNet(BasicBlock, [3, 4, 6, 3],num_classes=2)
    return model

if __name__ == '__main__':
    model = DualResNet(BasicBlock, [3, 4, 6, 3],num_classes=2)
    t = torch.rand(size=(2,3,384,384))
    out = model(t)
    print(out.size())